                                    ...
                                    <a-spin :spinning="spin" :delay="100">
                                          <div class="editor_border">
                                                <Editor container-id="apply" ref="editor" @getValues="testResults"
                                                      @changeContent="() => !enabled ? enabled = true : null" :readonly="true">
                                                </Editor>
                                          </div>
                                          <br />
...
const { orderProfileArch, editor, FetchProfileSQL } = FetchMixins()
...
const postOrder = debounce(() => {
      ...
      formRef.value.validate().then(() => {
            ...
            request.CopyOrderByIDC(wrapper).finally(() => enabled.value = true)
      }).catch(() => {
...
onMounted(() => {
      ...
      orderItems.work_id = route.query.work_id as string
      ...
      orderItems.data_base = store.state.order.order.data_base
      orderItems.table = store.state.order.order.table
      orderItems.text = store.state.order.order.text
      FetchProfileSQL(store.state.order.order.work_id).then((res: AxiosResponse<Res<{ [key: string]: string }>>) => {
            editor.value.ChangeEditorText(res.data.payload.sqls)
            editor.value.readonly = true
